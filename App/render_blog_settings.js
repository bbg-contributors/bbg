...
function render_comment_related(trigger_by_change = false) {
  ...
  if (
    document.getElementById("comment_do_not_use_any_comment_system")
      .selected === true
  ) {
  ...
  function show_specified_detail(name) {
    ...
    for (let k in mapping_table) {
      if (k == name)
        document.getElementById(mapping_table[k]).setAttribute("style", "");
      else
        document
          .getElementById(mapping_table[k])
  ...
  if (current_selection === "valine") {
    ...
    if (trigger_by_change) {
      whether_confirm_to_use_valine_with_public_apikey =
        dialog.showMessageBoxSync({
          message:
            langdata["CONFIRM_TEXT_WHEN_USING_VALINE_WITH_PUBLIC_APIKEY"][
              lang_name
            ],
...
module.exports = function () {
  document
    ...
    .insertAdjacentHTML("beforeend", getUiFileContent("blog_settings_ui.html"));

  document.getElementById("blog_settings_title").value = xssStrict(
    blog["博客标题"]
  );
  document.getElementById("blog_settings_description").value = xssStrict(
    blog["博客描述（副标题）"]
  );
  document.getElementById("blog_settings_content_license").value = xssStrict(
    blog["全站内容授权协议"]
  );
  ...
  document.getElementById("website_announcement_content").value = xssStrict(
    blog["网站公告"]
  );
  document.getElementById("blog_settings_url_of_background").value = xssStrict(
    blog["全局主题设置"]["若使用背景图像，设置为"][
      "若将某个url作为背景图像，这个url是"
    ]
  );
  document.getElementById("blog_settings_color_of_background").value =
    xssStrict(blog["全局主题设置"]["若使用纯色背景，颜色为"]);
  document.getElementById("blog_settings_live2d_widget_path").value = xssStrict(
    blog["全局主题设置"]["live2d-widget设置"]["widget路径"]
  );
  document.getElementById("blog_settings_live2d_widget_tips_path").value =
    xssStrict(blog["全局主题设置"]["live2d-widget设置"]["tips路径"]);
  ...
  `;
    ...
    if (blog["全局主题设置"]["若使用第三方主题，是否来自本地文件"] === false) {
      ...
      <button class="btn btn-primary" onclick="check_thirdparty_theme_update();"><i class="fa fa-refresh"></i> ${langdata["CHECK_UPDATE"][lang_name]}: ${blog["全局主题设置"]["若使用来自主题商店的第三方主题，则主题名为"]}</button>
    </b>
  
  `;
    }

    document.getElementById("isUsingThirdPartyTheme").insertAdjacentHTML(
      "beforeend",
      `
    <button class="btn btn-primary" onclick="config_third_party_theme();"><i class="fa fa-cogs"></i> 编辑主题配置文件</button>
    `
    );
  } else {
    document.getElementById("isUsingThirdPartyTheme").innerHTML = `
  ...
  if (blog["提供复制全文到剪贴板的选项"] === true)
    document.getElementById(
      "enable_copy_full_article_to_clipboard"
    ).checked = true;
  ...
  if (blog["搜索按钮边框颜色设置为暗色"]) {
    document.getElementById(
      "set_search_button_outline_into_dark_mode"
    ).checked = true;
  }
  ...
  if (blog["全局主题设置"]["live2d-widget设置"]["功能设置"]["switch-model"]) {
    document.getElementById(
      "live2d_widget_switch_model_enabled"
    ).checked = true;
  }
  if (blog["全局主题设置"]["live2d-widget设置"]["功能设置"]["switch-texture"]) {
    document.getElementById(
      "live2d_widget_switch_texture_enabled"
    ).checked = true;
  }
  ...
  document.getElementById("auto_rss_enabled").checked =
    blog["在对文章列表进行修改后触发rss生成"];
  document.getElementById("auto_sitemap_enabled").checked =
    blog["在对文章或页面列表进行修改后触发sitemap.txt生成"];

  document.getElementById("rewrite_baseurl").checked =
    blog["Markdown渲染配置"]["使用markdown文件所在目录作为baseurl"];
  document.getElementById("enable_image_viewer").checked =
    blog["Markdown渲染配置"]["在用户点击图片时显示图片查看器"];
  document.getElementById("resize_images").checked =
    blog["Markdown渲染配置"]["根据用户屏幕尺寸渲染图片尺寸"];
  ...
  function render_input_of_background() {
    ...
    case "bgimg_use_pure_color":
      document
        .getElementById("blog_settings_color_of_background")
        .setAttribute("style", "");
      document
        .getElementById("blog_settings_url_of_background")
        .setAttribute("style", "display: none;");
      document.getElementById("blog_settings_color_of_background").value =
    ...
    case "bgimg_use_url":
      ...
      document
        .getElementById("blog_settings_url_of_background")
        .setAttribute("style", "");
      document.getElementById("blog_settings_url_of_background").value =
          blog["全局主题设置"]["若使用背景图像，设置为"][
            "若将某个url作为背景图像，这个url是"
          ];
      break;
  ...
  if (blog["全局主题设置"]["是否使用纯色背景（优先级高于背景图像）"]) {
    document.getElementById("blog_settings_background_setting").value =
      "bgimg_use_pure_color";
    ...
    if (
      ...
      document.getElementById("blog_settings_background_setting").value =
        "bgimg_use_background_webp";
    } else if (
      blog["全局主题设置"]["若使用背景图像，设置为"]["将某个url作为背景图像"]
    ) {
      document.getElementById("blog_settings_background_setting").value =
        "bgimg_use_url";
    }
  ...
  document.getElementById("blog_settings_background_setting").onchange =
    function () {
      render_input_of_background();
    };
  ...
  if (blog["全局评论设置"]["启用valine评论"]) {
    ...
    if (blog["全局评论设置"]["valine设置"]["是否使用bbg公共评论服务"])
      document.getElementById(
        "comment_select_to_use_valine_with_public_apikey"
      ).selected = true;
  ...
  document.getElementById("blog_settings_comment_system_setting").onchange =
    function () {
